...
<h3>Description</h3>
With real log data flowing from Sprint 2, activate Sentinel's built-in detection capability and extend it with custom KQL rules tailored to Smartovate Ltd's environment — mass resource deletion and brute-force sign-in patterns.

<h3>US 3.1 — Activation of native detection rules</h3>

*As a SOC analyst, I want to activate analytics rules based on Microsoft templates, in order to quickly detect known threats with no development effort.*
**Priority:** High

| Acceptance criteria | Status | Evidence |
|---|---|---|
| At least 5 relevant rules activated | ✅ Done | Five Microsoft template-based analytics rules activated in Sentinel → Analytics (Active rules) |
| Rules generate incidents in Sentinel with appropriate severity | ✅ Done | At least one native rule fired and produced an incident with an assigned severity level, visible in Sentinel → Incidents |

<h4>Issue Encountered & Resolved</h4>
**Problem:** The Microsoft template for SSH brute-force detection is incompatible with the Linux VM's key-only authentication — the template expects password-based failed-login events, which never occur with SSH key auth.
<br />
**Resolution:** Documented as a known limitation rather than forced to "pass" artificially; equivalent brute-force detection coverage is provided by the custom Windows RDP-based rule built for US 3.2.
<br />
**Verification:** Confirmed the template rule produces no incidents against the Linux VM by design, not by misconfiguration.

<h3>US 3.2 — Creation of custom KQL detection rules</h3>

*As a security engineer, I want to create custom analytics rules in KQL, in order to detect behaviors specific to Smartovate Ltd's environment.*
**Priority:** High

| Acceptance criteria | Status | Evidence |
|---|---|---|
| Mass resource deletion detection rule created | ✅ Done | Custom KQL rule detecting 5+ deletions within a 10-minute window; live-tested and confirmed by triggering **incident #23** |
| Brute-force sign-in detection rule created | ✅ Done | Custom KQL rule detecting 5 failed sign-ins followed by a success within 30 minutes; live-tested against `vm-windows` RDP sign-in events |
| KQL queries optimized to run in under 2 minutes | ✅ Done | Both rules' underlying queries manually re-run in Logs; execution time confirmed under the 2-minute threshold |

<h4>Issue Encountered & Resolved</h4>
**Problem:** Windows RDP sign-in events record the account name in an inconsistent format between failed and successful attempts (e.g., domain-qualified vs. bare username), breaking the session-matching logic that ties a run of failures to the following success.
<br />
**Resolution:** Adjusted the KQL join/matching logic to normalize the account name field (e.g., stripping domain prefixes) before correlating failed and successful sign-in events.
<br />
**Verification:** Re-tested the brute-force scenario end-to-end (5 failures + 1 success) and confirmed the rule correctly correlated the events and generated an incident.

<h3>Program walk-through</h3>
<p align="center">

Five native analytics rules active: <br/>
<img src="docs/images/native-rules.png" height="80%" width="80%" alt="Five native analytics rules enabled"/>
<br />
<br />

Native rule incident with severity: <br/>
<img src="docs/images/native-incident.png" height="80%" width="80%" alt="Incident generated by native rule"/>
<br />
<br />

Custom mass-deletion rule logic (KQL): <br/>
<img src="docs/images/kql-massdeletion.png" height="80%" width="80%" alt="Mass deletion KQL rule"/>
<br />
<br />

Incident #23 — mass deletion triggered: <br/>
<img src="docs/images/incident23.png" height="80%" width="80%" alt="Incident 23 mass deletion"/>
<br />
<br />

Custom brute-force rule logic (KQL): <br/>
<img src="docs/images/kql-bruteforce.png" height="80%" width="80%" alt="Brute force KQL rule"/>
<br />
<br />

Brute-force incident triggered (5 failures + success): <br/>
<img src="docs/images/incident-bruteforce.png" height="80%" width="80%" alt="Brute force incident"/>
<br />
<br />

Query runtime confirmation (under 2 minutes): <br/>
<img src="docs/images/query-runtime.png" height="80%" width="80%" alt="KQL query runtime under 2 minutes"/>
...
| User Story | Completion |
|---|---|
| US 3.1 — Activate native detection rules | ✅ 100% |
| US 3.2 — Create custom KQL detection rules | ✅ 100% |

**Sprint 3 status: fully complete. One template-level limitation documented (SSH brute-force template incompatible with key-only auth, covered instead by the custom RDP-based rule); one KQL logic fix documented (RDP account-name normalization for failed/success correlation).**
